...
function ensureAllowedCharacters(val) {
  return String((this ? this.value : val) || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^\x20-\x7E]|/g, "")
    .replace(/\'/g, "");
...
const commonSchemas = {
  ...
  date: {
    ...
    autoValue: function () {
      ...
      return m
        ? moment(new Date(m[3], m[2] - 1, m[1])).toISOString()
  ...
  brl: {
    ...
    custom: function () {
      return this.value.toFixed(2).length - 1 <= 16
        ? this.value >= 0
          ? undefined
          : "Value cannot be zero or less than zero"
        : "Value exceeds the maximum allowed digits.";
    },
  },
  brl_fees: {
    type: Number,
  ...
  coin: {
    ...
    custom: function () {
      return this.value.toFixed(10).length - 1 <= 30
        ? this.value >= 0
          ? undefined
          : "Value cannot be zero or less than zero"
        : "Value exceeds the maximum allowed digits.";
  ...
  document: {
    ...
    autoValue: function () {
      if (this.value) {
        if (
          this.siblingField(this.key.split("_")[0] + "_identity_type").value ===
            "CPF" ||
          this.siblingField(this.key.split("_")[0] + "_identity_type").value ===
            "CNPJ"
        ) {
